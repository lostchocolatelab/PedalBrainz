...
void rainbowNew(){
  ...
  pixel.setBrightness(MaxBright);
  inner.setBrightness(MaxBright);
  
  strip.show(); // and update the NeoPixels to show it
  pixel.show(); // and update the NeoPixels to show it
  inner.show(); // and update the NeoPixels to show it
  ...
  for(uint8_t i=0; i<pixel.numPixels() * 3; i++) {
    sum += pixelPtr[i];
  }
  if(count == 1530) { // Every 256th frame, estimate & print current
    //Serial.print(BASECURRENT + (sum * LEDCURRENT + 32640) / 65280);
    Serial.println(" count "+String(count));
    count = 0; // Reset pixel accumulator
